...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $features = [];
    $possibleFeatures = [
        'air-condition', 'navigation-system', 'heated-seats', 'apple-carplay',
        'bluetooth', 'leather-seats', 'camera', 'android',
        'cruise-control', 'sunroof', 'keyless', 'sound-system'
    ];
    ...
    try {
        ...
        $stmt->execute([
            $userId, $make, $model, $year, $carType, $description, $dailyRate, 
            $location, $transmission, $seats, $featuresJson, $availableFrom, $availableUntil
        ]);
        ...
        // Create directory for this car if it doesn't exist
        ...
        foreach ($_FILES as $key => $file) {
            if (strpos($key, 'carImage') === 0 && $file['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($file['tmp_name'], $filePath)) {
                    ...
                    $stmt->execute([$carId, $filePath, $isPrimary]);
                    $imageCount++;
                    $imagePaths[] = $filePath;
                }
            }
